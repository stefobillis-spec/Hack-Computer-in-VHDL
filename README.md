...
Add the UART source files from `Third Party Sources`.
...
Add a Clocking Wizard connected to the board's clock source and configure the external interfaces as follows:
...
Run the program then paste the generated machine code from the Nand2Tetris Assembler into the program when prompted. Next, enter the total number of instructions in the program.
...
For example, if the program contains 0 to 127 instructions, set the constant data_length equal to 127;

---

## Step 4 — Rebuild the FPGA Design

After modifying `ROM.vhd`, save the file, run synthesis and implementation again, generate a new bitstream, and program the FPGA with the new bitstream.
